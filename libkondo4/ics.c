 ...
 * Convenience macros
 */
#define ics_error(ki, func, err) { \
  snprintf(ki->error, 128, "ERROR: %s: %s\n", func, err); \
  return -1; }
#define ics_ftdi_error(ki, func) { \
  snprintf(ki->error, 128, "ERROR: %s: %s\n", func, \
...
int ics_read_timeout(ICSData * r, int n, long usecs)
{
	...
	do {
		if ((i = ftdi_read_data(&r->ftdic, r->swap, n - bytes_read)) < 0)
			ics_ftdi_error(r, "ics_read_timeout");
		bytes_read += i;
		gettimeofday(&tv, NULL);
...
int ics_pos(ICSData * r, UINT id, UINT pos)
{
	...
	if (id > 31)
		ics_error(r, "ics_pos", "Invalid servo ID > 31.");
	if (pos > 16383)
		ics_error(r, "ics_pos", "Invalid servo position > 16383.");
...
int ics_get_stretch(ICSData * r, UINT id)
{
	...
	// check valid id
	if (id > 31)
		ics_error(r, "ics_get_stretch", "Invalid servo ID > 31.");
...
int ics_get_speed(ICSData * r, UINT id)
{
	...
	// check valid id
	if (id > 31)
		ics_error(r, "ics_get_speed", "Invalid servo ID > 31.");
...
int ics_get_current(ICSData * r, UINT id)
{
	...
	// check valid id
	if (id > 31)
		ics_error(r, "ics_get_current", "Invalid servo ID > 31.");
...
int ics_set_stretch(ICSData * r, UINT id, UCHAR stretch)
{
	...
	// check valid stuff
	if (id > 31)
		ics_error(r, "ics_set_stretch", "Invalid servo ID > 31.");
	if (stretch < 1 || stretch > 127)
		ics_error(r, "ics_set_stretch", "Invalid stretch, not [1-127]");
...
int ics_set_speed(ICSData * r, UINT id, UCHAR speed)
{
	...
	// check valid stuff
	if (id > 31)
		ics_error(r, "ics_set_speed", "Invalid servo ID > 31.");
	if (speed < 1 || speed > 127)
		ics_error(r, "ics_set_speed", "Invalid speed, not [1-127]");
...
int ics_set_current_limit(ICSData * r, UINT id, UCHAR curlim)
{
	...
	// check valid stuff
	if (id > 31)
		ics_error(r, "ics_set_current_limit", "Invalid servo ID > 31.");
	if (curlim > 1 || curlim > 63)
		ics_error(r, "ics_set_current_limit", "Invalid current, not [1-63]");
...
int ics_set_id(ICSData * r, UINT id)
{
	...
	// check id
	if (id > 31)
		ics_error(r, "ics_set_id", "Invalid servo ID, > 31.");
